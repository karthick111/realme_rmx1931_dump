...
function remove_lager_hang_log()
{
    if [ ! -d ${PHX_LOG_DIR} ];
    then
        echo "remove_lager_hang_log:No such file or directory:$PHX_LOG_DIR"
        return 0;
    fi

    if [ "x$PHX_LOG_DIR" == "x" ] ;
    then
        echo "PHX_LOG_DIR is NULL"
        return 0;
    fi

    lager_files_remove_count=0
    for lager_files in $(find $PHX_LOG_DIR -size +10M)
    do
        let "lager_files_remove_count=lager_files_remove_count+1"
        if [ $lager_files_remove_count -gt 2 ]
        then
            echo "Delete up to 2 files,$lager_files_remove_count"
            return 0;
        fi
        echo "remove_lager_hang_log:rm $(ls -s $lager_files)"
        rm $lager_files
    done
}
